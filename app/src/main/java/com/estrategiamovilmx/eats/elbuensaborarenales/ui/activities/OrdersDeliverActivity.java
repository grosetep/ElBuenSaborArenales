        ...
        if (user!=null) {
            switch (user.getProfile()){
                case Constants.profile_admin:
                    ...
                    adapter.addFragment(PendingOrdersFragment.newInstance(ALL_ORDERS_ADMIN), "ADMINISTRAR PEDIDOS");
                    break;
                case Constants.profile_client:Log.d("ordenesdebug","entra en client");
                    adapter.addFragment(PendingOrdersFragment.newInstance(ALL_ORDERS_CLIENT), "MIS PEDIDOS");
                    break;
